    ...
    Sub New(ByVal NroSoli As Integer, ByVal Paridad As Double)
        ...
        Try
            Dim SQLCnlst As String = "SELECT s.NroSolicitud, s.Solicitante, Tipo = IIF(s.Tipo = 1, 'Pago Prov.',  'Varios'), " _
                                 & "Destino = IIF(s.Proveedor = '',c.Descripcion, p.Nombre), s.Titulo, s.Concepto, s.Concepto_Pago, " _
                                 & "Moneda = IIF(s.Moneda = 2, 'U$D',  '$'), s.TipoDolar, s.Importe, s.Proveedor, s.Cuenta, s.ObservacionesPago, " _
            ...
            If RowSoli IsNot Nothing Then
                With RowSoli
                    ...
                    txt_DetalleAgregadoPorSergio.Text = Trim(IIf(IsDBNull(.Item("Concepto")), "", .Item("Concepto")))
