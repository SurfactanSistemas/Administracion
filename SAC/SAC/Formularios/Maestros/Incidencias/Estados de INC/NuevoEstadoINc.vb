Public Class NuevoEstadoINC
    ...
    Private Function _ProximoNumero() As String
        Dim WUltimo As DataRow = GetSingle("SELECT ISNULL(Max(Estado), 0) Ultimo FROM EstadosINC")
    ...
    Private Sub txtCodigo_KeyDown(ByVal sender As Object, ByVal e As KeyEventArgs) Handles txtCodigo.KeyDown
        ...
        If e.KeyData = Keys.Enter Then
            ...
            Dim WTipoSolic As DataRow = GetSingle("SELECT Estado, LTRIM(RTRIM(Descripcion)) Descripcion FROM EstadosINC WHERE Estado = '" & txtCodigo.Text & "'")

            If Not IsNothing(WTipoSolic) Then
                txtCodigo.Text = WTipoSolic.Item("Estado")
    ...
    Private Sub btnGrabar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnGrabar.Click
        Try
            ...
            ExecuteNonQueries("DELETE FROM EstadosINC WHERE Estado = '" & txtCodigo.Text & "'", "INSERT INTO EstadosINC (Estado, Descripcion) VALUES (" & txtCodigo.Text & ", '" & txtDescripcion.Text.Trim.SliceLeft(30) & "')")

            Dim WOwner = TryCast(Owner, INuevoEstadoINC)

            If Not IsNothing(WOwner) Then WOwner._ProcesarNuevoEstadoINC(txtCodigo.Text, txtDescripcion.Text)
    ...
    Private Sub btnEliminar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnEliminar.Click
        Try
            If MsgBox("¿Está seguro de querer eliminar este Estado?", MsgBoxStyle.YesNoCancel) <> MsgBoxResult.Yes Then Exit Sub

            ExecuteNonQueries("DELETE FROM EstadosINC WHERE Estado = '" & txtCodigo.Text & "'")
